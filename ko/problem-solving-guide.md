...
에이전트 연결을 완료한 뒤 NHN Cloud 인스턴스에 접근하려면 몇 가지 추가 설정이 필요합니다. 아래 항목을 꼭 확인하세요.

* 라우트 설정
    * 자세한 내용은 사용자 가이드 - 시작하기를 참고하세요.
* ACL 정책 설정
    * 에이전트를 통해 내부 인스턴스에 접근할 때 적용되는 접근 제어 정책입니다.
* Security Groups 설정
    * 인스턴스에 설정된 보안 그룹에서 출발지 IP를 허용해야 합니다.

<br>

## 윈도우 사용자인데 생체 인증이 적용되지 않습니다.
Cloud Access의 생체 인증은 데스크탑이나 노트북에 내장된 생체 인증 정보를 활용합니다. 생체 인증이 없는 디바이스의 경우 **계정 - 로그인** 옵션에서 PIN을 먼저 설정한 뒤 생체 인증을 사용할 수 있습니다.

<br>

## 사용자 계정이 생성되어 있는데 사용자 객체 추가 버튼을 누르면 항목이 나타나지 않습니다.
사용자 계정을 생성한 직후에는 아직 IP 정보가 없어 사용자 객체 목록에 표시되지 않습니다. 해당 계정이 로그인하여 IP를 할당 받은 후 목록에서 확인 가능합니다.
...
현재 Cloud Access서비스가 비활성화 상태이므로 사용자 계정 연결을 할 수 없습니다. 서비스를 활성화한 후 다시 연결 항목을 추가하거나 불필요한 항목이라면 삭제하세요.
