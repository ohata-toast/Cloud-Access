...
**정책** 탭에서는 외부에서 연결된 에이전트와 내부 인스턴스 간 트래픽을 제어하는 **ACL정책**과 인증 및 엔드포인트 설정 등 에이전트에 적용할 정책을 그룹별로 적용할 수 있는 **사용자 정책**을 관리합니다.
...
* ➋ 정책에 적용할 출발지와 목적지 객체를 선택합니다.
    * 이미 등록된 객체 중에서 선택할 객체가 없다면 **객체 추가** 버튼을 클릭해 새로 생성할 수 있습니다.
