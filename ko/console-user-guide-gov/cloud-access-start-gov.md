...
위와 같이 설정되어 있을 때 VPC1(로컬)과 VPC2(피어)간 피어링을 설정합니다. 그리고 **Network - Routing**에서 접속 필요한 인스턴스가 속한 라우팅 테이블을 선택하여 **라우트** 탭에 아래와 같은 규칙을 추가합니다.

* 대상 CIDR: 10.0.0.0/24
* 게이트웨이: VPC1과 VPC2 사이에 생성된 PEERING 타입의 네트워크 인터페이스

그리고 **Peering Gateway - 피어링**에서 **라우트** 탭을 선택하여 로컬 라우트 규칙을 추가합니다.
...
위와 같이 설정되어 있을 때 1번 프로젝트(로컬)와 2번 프로젝트(피어) 간 피어링을 설정합니다. 그리고 **Network - Routing**에서 접속 필요한 인스턴스가 속한 라우팅 테이블을 선택하여 **라우트** 탭에 아래와 같은 규칙을 추가합니다.

* 대상 CIDR: 10.0.0.0/24
* 게이트웨이: 프로젝트 1번 VPC와 프로젝트 2번 VPC 사이에 생성된 PEERING 타입의 네트워크 인터페이스


그리고 **Peering Gateway - 프로젝트 피어링**에서 **라우트** 탭을 선택하여 로컬 라우트 규칙을 추가합니다.
...
!!! danger "주의"
    ...
    * 사용자 IP 할당 대역은 아래의 항목과 겹칠 수 없습니다.
        * 연결 설정 시 선택한 서브넷
        * 접근 가능 대역
    * Security Groups에서 사용자 IP 할당 대역을 허용해야 통신 가능합니다.
